...
def clear(version, whitelist, blacklist, force, default_registry):
    try:
        client = docker.from_env()
        images = __getImages(client, version, default_registry)

        for img in __filter(images, whitelist, blacklist, default_registry):
...
def __filter(images, whitelist, blacklist, default_registry):
    tags = dict()
    prefix = "ignishpc/"
    for img in images:
